...
OUT_DIR = PROJECT_ROOT / "outputs" / "oct" / "dip_full"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run DIP on the full OCT B-scan.")
...
def save_loss_curve(losses: list[float], path: Path) -> None:
    ...
    plt.title("DIP training loss (full OCT B-scan)")
    plt.tight_layout()
...
def main() -> None:
    ...
    print("Step 1. Load the full 8-bit HR B-scan (no crop).")
    hr = np.asarray(Image.open(HR_BSCAN_PATH).convert("RGB"))
    height, width = hr.shape[:2]
    print(f"  {HR_BSCAN_PATH.name}: shape {hr.shape} (W×H = {width}×{height})")
    if width % SCALE != 0 or height % SCALE != 0:
        raise ValueError(
            f"B-scan {width}×{height} must divide by scale {SCALE}."
        )
    Image.fromarray(hr).save(OUT_DIR / "hr.png")

    print(f"\nStep 2. Shrink the full B-scan by x{SCALE} with the DIP bicubic operator.")
    ...
    print("\nStep 3. Bicubic baseline: enlarge LR back to full HR size.")
    bicubic = from_tensor(bicubic_upsample(lr_tensor, SCALE))
    Image.fromarray(bicubic).save(OUT_DIR / "bicubic.png")

    print("\nStep 4. Score bicubic vs the full HR B-scan (this is the number DIP must beat).")
    bicubic_scores = evaluate(hr, bicubic)
    print(
    ...
    print(f"\nStep 5. Train DIP for {args.iterations} iterations on {device}.")
    print("  I do not show the HR B-scan to the network. Stopping rule is fixed iterations.")
    print(f"  Network output size will be {height}×{width} (full slice).")
    ...
    print("\nStep 6. Score DIP vs the same full HR B-scan.")
    dip_scores = evaluate(hr, dip)
    print(
    ...
    for iteration in checkpoint_iterations:
        ...
        checkpoint_scores = evaluate(hr, np.asarray(checkpoint_image))
    ...
    save_comparison(hr, bicubic, dip, OUT_DIR / "comparison.png")
    ...
    lines = [
        "OCT DIP vs bicubic, TX12_D0_A1L slice 256, FULL B-scan (no crop), x8",
        ...
        "Note: these scores are for the full 512x1024 B-scan, not the earlier 256 crop.",
    ]
